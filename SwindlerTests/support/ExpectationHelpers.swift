...
func waitUntil( _ expression: @autoclosure @escaping () throws -> Bool, file: String = #file, line: UInt = #line) {
  expect(try expression(), file: file, line: line).toEventually(beTrue())
}

func waitFor<T>( _ expression: @autoclosure @escaping () throws -> T?, file: String = #file, line: UInt = #line) -> T? {
...
func it<T>(_ desc: String, timeout: TimeInterval = 1.0, failOnError: Bool = true, file: String = #file, line: UInt = #line, closure: @escaping () -> Promise<T>) {
...
func expectToSucceed<T>(_ promise: Promise<T>, file: String = #file, line: UInt = #line) -> Promise<Void> {
  return promise.asVoid().recover { (error: Error) -> () in
    fail("Expected promise to succeed, but failed with \(error)", file: file, line: line)
  }
}

func expectToFail<T>(_ promise: Promise<T>, file: String = #file, line: UInt = #line) -> Promise<Void> {
...
func expectToFail<T, E: Error>(_ promise: Promise<T>, with expectedError: E, file: String = #file, line: UInt = #line) -> Promise<Void> {
  return promise.asVoid().then {
